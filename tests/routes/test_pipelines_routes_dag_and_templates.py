"""Tests for pipeline route DAG, template, dry-run, and AI-edit paths."""
...
async def test_api_channels_search_short_query(route_client, base_app):
    ...
    resp = await route_client.get("/pipelines/api/channels/search?q=a")
...
async def test_api_channels_search_with_query(route_client, base_app):
    ...
    resp = await route_client.get("/pipelines/api/channels/search?q=UniqueSearch")
    assert resp.status_code == 200
    data = resp.json()
...
async def test_api_channels_search_by_channel_id(route_client, base_app):
    ...
    resp = await route_client.get("/pipelines/api/channels/search?q=12345")
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) >= 1
...
async def test_api_channels_search_result_fields(route_client, base_app):
    ...
    resp = await route_client.get("/pipelines/api/channels/search?q=Fi")
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) >= 1
    item = data[0]
...
async def test_create_wizard_page_renders(route_client, base_app):
    ...
    app, _, _ = base_app
    app.state.llm_provider_service = _make_provider_svc(has=True)
    resp = await route_client.get("/pipelines/create")
    assert resp.status_code == 200
...
async def test_dry_run_count_dag_pipeline_falls_back_to_sidecar_sources(route_client, base_app):
    ...
    resp = await route_client.get(f"/pipelines/{pipeline_id}/dry-run-count")
    assert resp.status_code == 200
...
async def test_dry_run_count_new(route_client, base_app):
    ...
    app, _, _ = base_app
    app.state.llm_provider_service = _make_provider_svc()
    resp = await route_client.get("/pipelines/dry-run-count?source_ids=100&since_value=6&since_unit=h")
    assert resp.status_code == 200
    data = resp.json()
    assert "total" in data
    assert "after_filter" in data
...
async def test_dry_run_count_new_empty_ids(route_client, base_app):
    ...
    app, _, _ = base_app
    app.state.llm_provider_service = _make_provider_svc()
    resp = await route_client.get("/pipelines/dry-run-count?source_ids=&since_value=6&since_unit=h")
    assert resp.status_code == 200
    data = resp.json()
    assert data["total"] == 0
...
async def test_templates_json(route_client, base_app):
    ...
    with patch("src.web.routes.pipelines.deps.pipeline_service") as mock_svc:
        ...
        resp = await route_client.get("/pipelines/templates/json")
    assert resp.status_code == 200
    data = resp.json()
    assert isinstance(data, list)
    assert len(data) == 1
...
async def test_get_refinement_steps_not_found(route_client, base_app):
    ...
    resp = await route_client.get("/pipelines/999/refinement-steps", follow_redirects=False)
    assert resp.status_code == 303
...
async def test_pipelines_page_with_no_llm_provider(route_client, base_app):
    ...
    resp = await route_client.get("/pipelines/")
    assert resp.status_code == 200
