...
class TestGlobalOptions:
    ...
    def test_help_flag_shows_usage(self):
        """--help exits cleanly and shows the app's (short, unwrapped) help text."""
        result = runner.invoke(app, ["--help"])
        assert result.exit_code == 0
        # Only assert on text short enough that Rich never line-wraps it — the
        # rendered help is terminal-width dependent (Rich injects soft wraps and
        # ANSI between characters at narrow widths, which broke an `--config`
        # substring match under CI's 80-col terminal). Option presence is
        # asserted structurally below instead of by scraping the Rich output.
        assert "TG Post Search" in result.output

    def test_help_advertises_global_options(self):
        """--config / --version are registered options (terminal-independent).

        Introspect the Click command Typer builds rather than the Rich-rendered
        help string: the latter wraps/ANSI-escapes at narrow terminal widths, so
        a substring match on it is flaky in CI. The command's params are the
        source of truth for what help advertises.
        """
