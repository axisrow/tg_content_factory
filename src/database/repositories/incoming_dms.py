...
`dialogs_history`: конечный TTL, prune при каждой записи, чтобы приватный
текст не оставался в БД дольше TTL, даже если новых сообщений не приходит.
"""
...
class IncomingDmsRepository:
    ...
    async def record(
        ...
        dialogs_history): запись без prune копила бы приватный текст вечно,
        как только поток сообщений иссякнет.
        """
