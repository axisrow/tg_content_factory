...
def register(db, client_pool, embedding_service, **kwargs):
    ...
    async def export_session(args):
        ...
        except AccountSessionDecryptError as e:
            # Clean, actionable message (parity with CLI #1143); carries only phone+status.
            return _text_response(f"Не удалось расшифровать сессию аккаунта (status={e.status}).")
        except Exception:
            # Never interpolate the exception into the reply — the operation handles a
            # secret and a future stack change must not be able to surface it (#828).
            logger.exception("export_session failed for account_id=%s", account_id)
            return _text_response("Ошибка экспорта сессии (подробности в логах сервера).")
    ...
    async def import_session(args):
        ...
        try:
            ...
            # Don't echo the exception — it handles a session string (secret) (#828).
            logger.exception("import_session failed for phone=%s", phone)
            return _text_response("Ошибка импорта сессии (подробности в логах сервера).")
