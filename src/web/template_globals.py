...
FLASH_MESSAGES = {
    ...
    "account_deleted_notify_reassigned": "Аккаунт удалён. Уведомления переназначены на другой аккаунт.",
    "account_deleted_notify_cleared": "Аккаунт удалён. Аккаунт уведомлений сброшен — используется Primary.",
    ...
    "queue_paused": "Очередь сбора поставлена на паузу. Поставленные задачи ждут возобновления.",
    "queue_resumed": "Очередь сбора возобновлена.",
    ...
    "filter_analyze_queued": "Анализ фильтров запущен в фоне.",
...
FLASH_ERRORS = {
    ...
    "filter_analyze_running": "Анализ фильтров уже выполняется.",
    ...
    "invalid_notify_account": "Аккаунт для переназначения уведомлений не найден. Аккаунт не удалён.",
    ...
    "hard_delete_confirm_required": (
        "Подтверждение удаления не прошло проверку. Введите DELETE_ALL_FILTERED в поле подтверждения."
    ),
    "hard_delete_set_changed": (
        "Список отфильтрованных каналов изменился с момента загрузки страницы. "
        "Обновите страницу и повторите подтверждение."
    ),
    "hard_delete_partial": (
        "Удаление выполнено частично. Часть каналов осталась в базе — проверьте список ниже "
        "и повторите попытку для оставшихся."
    ),
    "purge_partial": (
        "Очистка выполнена частично — сообщения части каналов удалить не удалось. "
        "Проверьте логи и повторите попытку."
    ),
