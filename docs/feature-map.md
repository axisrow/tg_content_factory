...
| CLI | ✅ | `photo-loader dialogs/refresh/send/schedule-send/batch-*/publish/items/auto-*/run-due` |
| Agent-tools | ✅ | `send_photos_now`, `schedule_photos`, `create_photo_batch`, `publish_photo_batch`, `create_auto_upload`, `run_photo_due`… |
| TUI | — | |
...
| Agent-tools | ✅ | 15 tools (14 read + `rate_channel` write): `get_analytics_summary`, `get_top_messages`, `get_hourly_activity`, `get_peak_hours`, `get_channel_analytics`, `get_trending_*`… |
| TUI | — | |
...
| Agent-tools | ✅ | `agent_threads.py` (5: list/create/delete/rename/get_messages) + сам реестр 173 tools |
...
**Статус:** 🟡 **только код** — лимиты подключены, но off-by-default; `ErrorRecoveryService` изолирован. Реальная работа лимитов боем не доказана.
**Vulture-аудит #1044:** `ErrorRecoveryService` верифицирован как изолированный (grep `src/` = только сам файл; `CircuitBreaker` в `notifier.py` — это pybreaker #955, другая реализация; нет DI/динамической регистрации). Решение владельца — **НЕ удалять, подключить как фичу** (отдельная задача, не #1044). ⚠️ При подключении сверять с #958: `execute_with_recovery` ретраит любую `func` (max_retries=3) → подключать только к idempotent-операциям, НЕ к платным image POST (осознанный `max_retries=0` против дубль-биллинга).
...
**Vulture-аудит #1044:** изоляция верифицирована (grep `src/` = только сам файл; нет DI/динамической регистрации; репо-методы `generation_runs.set_variants`/`select_variant` зовутся ТОЛЬКО из `ABTestingService` — ни web, ни CLI вариантов не показывают). Решение владельца — **НЕ удалять, подключить как фичу** (отдельная задача-эпик «подключить неиспользуемое», не #1044).
...
Каждый из 26 сервисов получает РОВНО один общий статус (по основному, обычно read-, пути). Числа ниже складываются в 26.

| Статус | Кол-во | Сервисы |
|--------|--------|---------|
| 🟢 **проверено вживую** | **21** | Сбор (1), Поиск (2, base FTS+live-TG), Каналы (3), Очистка/анализ (4), Пайплайны-чтение (8), Модерация-чтение (9), Публикация текста (10), Постинг картинок (11), Диалоги (12), Аккаунты-чтение (13), Уведомления-статус (14), Search-queries-чтение (15), Аналитика+Тренды (16), Календарь (17), Планировщик (18), Jobs (19), Перевод-stats (20), Экспорт (21), Провайдеры-list (22), Агент-чтение (23), Настройки (24) |
| 🟡 **только код** | **4** | Рейтинг каналов (5), AI-генерация (6), Генерация картинок (7), Production Limits & Quality (25) |
| 🔴 **заглушка/частично** | **1** | A/B-тестирование (26); внутри #25 `ErrorRecoveryService` тоже мёртв |

**Итого: 21 + 4 + 1 = 26.**

> ⚠️ Числа выше — по ОБЩЕМУ статусу сервиса. Отдельно от них: многие из 21 «зелёных» сервисов **гибридные** — read-путь 🟢, а write/мутирующий путь 🟡 (помечено в строке статуса каждой секции). Это НЕ отдельная категория в таблице (иначе сумма сломается) — это под-статус внутри зелёных. «Тёмные половины» (деструктив/auth/BotFather/provider-проба) у сервисов 4/9/13/14/20/22: read тривиально гоняется в `safe_ro`, мутации намеренно вынесены из авто-suite. Для баг-ханта именно эти write-половины — приоритет.
...
8. **A/B-тестирование (26)** — 🔴 не подключён ни к одной поверхности. Vulture-аудит #1044 подтвердил изоляцию; решение владельца — **подключить как фичу** (отдельная задача-эпик), НЕ удалять. Пока — bit-rot.
9. **ProductionLimits / ErrorRecovery (25)** — `ErrorRecoveryService` не импортируется; `ProductionLimitsService` off-by-default. Vulture-аудит #1044 подтвердил изоляцию `ErrorRecoveryService`; решение владельца — **подключить как фичу** (отдельная задача), НЕ удалять (⚠️ retry сверять с #958). Если лимиты реально нужны — по умолчанию не работают (риск перерасхода на провайдерах).
