...
**In CI (#1090, #1097):**
- **PR** → `pytest --testmon` single-process (selective, no coverage). The `.testmondata` baseline is cached **per PR branch**: the first push builds it (runs everything once), later pushes to the same PR re-run only affected tests. A cache miss just rebuilds the baseline that run (safe — no false green). This is the fast, blocking PR test gate.
